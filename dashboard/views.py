...
def update_homework(request, pk=None):
    ...
    return redirect('homework')
@login_required
def update_homework1(request, pk=None):
    homework = Homework.objects.get(id=pk)
    if homework.is_finished == True:
        homework.is_finished = False
    else:
...
def update_todo(request, pk=None):
    ...
    return redirect('todo')
@login_required
def update_todo1(request, pk=None):
    todo = Todo.objects.get(id=pk)
    if todo.is_finished == True:
        todo.is_finished = False
    else:
...
def books(request):
    if request.method=='POST':
        ...
        for i in range(10):
            ...
            'description':answer['items'][i]['volumeInfo'].get('description','Not Available'),
            'pages':answer['items'][i]['volumeInfo'].get('pageCount','Not Available'),
            'categories':answer['items'][i]['volumeInfo'].get('categories'),
            'rating':answer['items'][i]['volumeInfo'].get('pageRating'),
            'thumbnail':answer['items'][i]['volumeInfo'].get('imageLinks',{}).get('thumbnail','Not Available'),
            'preview':answer['items'][i]['volumeInfo'].get('previewLink','Not Available'),
            'author':answer['items'][i]['volumeInfo'].get('authors','Not Available'),

            }
            result_list.append(result_dict)
        context={'form':form,'books':result_list}
...
def wikipedia_search(request):
    if request.method=='POST':
        ...
        context={'form':form,'title':search.title,'link':search.url,'summary':search.summary,'results':True}
        return render(request, 'dashboard/wikipedia.html',context)
    else:
        form=DashboardForm()
    context={'form':form,'results':False}
    return render(request, 'dashboard/wikipedia.html',context)
            ...
            if 'input' in request.POST:
                ...
                if input and float(input) >= 0:
                    ...
                    if first == 'yard' and second == 'foot':
                        answer = f'{input} yard = {val * 3} foot'
                    if first == 'foot' and second == 'yard':
                        answer = f'{input} foot = {val / 3} yard'

                    # Added conversions
                    if first == 'meter' and second == 'centimeter':
                        answer = f'{input} meter = {val * 100} centimeter'
                    if first == 'centimeter' and second == 'meter':
                        answer = f'{input} centimeter = {val / 100} meter'

                    if first == 'inch' and second == 'centimeter':
                        answer = f'{input} inch = {val * 2.54} centimeter'
                    if first == 'centimeter' and second == 'inch':
                        answer = f'{input} centimeter = {val / 2.54} inch'

                    if first == 'foot' and second == 'inch':
                        answer = f'{input} foot = {val * 12} inch'
                    if first == 'inch' and second == 'foot':
                        answer = f'{input} inch = {val / 12} foot'

                context = {'form': form, 'm_form': measurement_form, 'input': True, 'answer': answer}

        if request.POST['measurement'] == 'mass':
            measurement_form = ConversionMass()
            context = {'form': form, 'm_form': measurement_form, 'input': True}
            if 'input' in request.POST:
                first = request.POST['measure1']
                second = request.POST['measure2']
                ...
                if input and float(input) >= 0:
                    ...
                    if first == 'pound' and second == 'kilogram':
                        answer = f'{input} pound = {val * 0.453592} kilogram'
                    if first == 'kilogram' and second == 'pound':
                        answer = f'{input} kilogram = {val * 2.20462} pound'

                    # Added conversions
                    if first == 'gram' and second == 'kilogram':
                        answer = f'{input} gram = {val / 1000} kilogram'
                    if first == 'kilogram' and second == 'gram':
                        answer = f'{input} kilogram = {val * 1000} gram'

                    if first == 'ounce' and second == 'gram':
                        answer = f'{input} ounce = {val * 28.3495} gram'
                    if first == 'gram' and second == 'ounce':
                        answer = f'{input} gram = {val / 28.3495} ounce'
